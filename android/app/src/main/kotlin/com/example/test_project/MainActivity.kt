...
class MainActivity : FlutterActivity() {
    ...
    // ── Download directo no 1DM — sem abrir o browser interno ────────────
    private fun openWith1DM(url: String, filename: String): Boolean {
        ...
        for (pkg in packages) {
            ...
            try {
                // setDataAndType com "application/octet-stream" força o 1DM
                // a abrir o ecrã de download directo em vez do browser interno
                val intent = Intent(Intent.ACTION_VIEW).apply {
                    setPackage(pkg)
                    setDataAndType(Uri.parse(url), "application/octet-stream")
                    putExtra("url", url)
                    putExtra("filename", filename)
                    putExtra("title", filename)
                    putExtra("userAgent", "Mozilla/5.0 (Linux; Android 12) AppleWebKit/537.36 Chrome/112.0 Mobile Safari/537.36")
                    putExtra("referUrl", url)
                    addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
                }
                startActivity(intent)
                return true
            } catch (e: Exception) { /* tenta próximo pacote */ }
        }
        return false
    }
