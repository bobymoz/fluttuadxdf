...
const String _xAppData = "";

const Map<String, String> _smartPlayHeaders = {
...
Future<List<Map<String, String>>> fetchScraperData(String url, {String? filterType}) async {
  ...
  try {
    final res = await http.get(Uri.parse(url), headers: _smartPlayHeaders);
...
class _PlayerScreenState extends State<PlayerScreen> {
  ...
  Future<void> _abrirServidores(String idVideo, String nomeVideo, bool isParaDownload) async {
    ...
    if (!isParaDownload) {
      ...
      setState(() { 
        ...
        savedEpId = idVideo;
        // Limpa chips do episódio anterior para não mostrar opções enquanto carrega
        _serversDisponiveis = [];
        _urlAtiva = '';
        _idiomaAtivo = '';
      });
    ...
    try {
      final res = await http.post(Uri.parse(urlApi), headers: _smartPlayHeaders, body: json.encode(payload)).timeout(const Duration(seconds: 60));
      ...
      if (data['success'] == true && data['players'] != null) {
        ...
        // Guarda localmente — o setState virá só depois do player inicializar
      ...
      if (!isParaDownload) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Sem resposta do servidor. Verifique a sua conexão."), backgroundColor: Colors.orange),
          );
        }
      }
    }
  ...
  Widget _chipServidor(Map server, String nomeVideo) {
    ...
    return GestureDetector(
      ...
      onTap: () {
        ...
        _playerInitializing = false; // permite nova inicialização ao trocar idioma
  ...
  void _iniciarExoPlayer(String url, String tituloEpisodio, {VideoPlayerController? controllerPreinit}) async {
    // Guarda contra chamadas concorrentes (ex: dois servidores ao mesmo tempo)
    ...
    // Detecta formato pelo URL para suportar MKV, MP4, HLS, etc.
    VideoFormat? formatHint;
    final urlLow = url.toLowerCase();
    if (urlLow.contains('.m3u8') || urlLow.contains('hls')) {
      formatHint = VideoFormat.hls;
    } else if (urlLow.contains('.mkv') || urlLow.contains('.mp4') || urlLow.contains('.avi') || urlLow.contains('.mov') || urlLow.contains('.webm')) {
      formatHint = VideoFormat.other;
    } else {
      formatHint = VideoFormat.other; // tenta reproduzir tudo
    }
    ...
    try {
      ...
      _videoPlayerController!.addListener(() {
        ...
        if (buf != _isBuffering) {
          if (buf) {
            // Buffering iniciou: espera 3s antes de mostrar o indicador (evita falso positivo ao arrastar)
            _bufferDebounce?.cancel();
            _bufferDebounce = Timer(const Duration(seconds: 3), () {
              if (mounted && _videoPlayerController != null && _videoPlayerController!.value.isBuffering) {
                setState(() => _isBuffering = true);
              }
            });
          } else {
            // Buffering terminou: cancela debounce e esconde imediatamente
            _bufferDebounce?.cancel();
      ...
      if (mounted) setState(() { 
        ...
        // _serversDisponiveis já foi atribuído — este setState torna os chips visíveis
        // só agora que o player está pronto e o layout está estável
      });
      ...
      if (mounted) {
        ...
        final msg = e.toString().contains('TimeoutException')
            ...
            : "Erro ao carregar o vídeo. Tenta outra opção.";
        ScaffoldMessenger.of(context).showSnackBar(
          ...
          children: [
            ...
            Column(
              children: [
                ...
                // InAppWebView SEMPRE no tree (nunca destruído) — altura 0 para não afetar layout
                SizedBox(
                  height: 0,
                  width: 0,
                  child: OverflowBox(
