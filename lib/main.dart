...
class _PlayerScreenState extends State<PlayerScreen> {
  ...
  void _iniciarExoPlayer(String url, String tituloEpisodio, {VideoPlayerController? controllerPreinit}) async {
    ...
    try {
      ...
      // Verifica se o vídeo inicializou corretamente (multi-track pode causar tela cinza)
      if (!_videoPlayerController!.value.isInitialized) {
        throw Exception("Vídeo não inicializado corretamente.");
      }

      // Aguarda até o tamanho do vídeo estar disponível (evita tela cinza em streams multi-áudio)
      if (_videoPlayerController!.value.size == Size.zero) {
        for (int i = 0; i < 20; i++) {
          await Future.delayed(const Duration(milliseconds: 150));
          if (_videoPlayerController!.value.size != Size.zero) break;
        }
      }
    ...
    return Focus(
      ...
      child: Stack(
        ...
        children: [
          ...
          // Player Chewie — só renderiza quando completamente pronto E com tamanho válido
          if (isPlaying && !isServerLoading && _chewieController != null &&
              (_videoPlayerController?.value.isInitialized ?? false) &&
              (_videoPlayerController?.value.size != Size.zero))
