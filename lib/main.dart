...
class _PlayerScreenState extends State<PlayerScreen> {
  ...
  Widget _buildSeletorIdioma(List<Map> servers, String nomeVideo) {
    if (servers.length < 2) return const SizedBox.shrink(); // só mostra se houver mais de 1 opção
    ...
    return Column(
      ...
      children: [
        ...
        Row(children: [
          Container(width: 4, height: 18, color: const Color(0xFFE50914), margin: const EdgeInsets.only(right: 8)),
          const Text("Opções de Áudio", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 14)),
        ]),
        const SizedBox(height: 8),
        Wrap(
          spacing: 8,
          runSpacing: 8,
          children: servers.map((server) => _chipServidor(server, servers, nomeVideo)).toList(),
        ),
        const SizedBox(height: 8),
      ],
    );
  ...
  Widget _chipServidor(Map server, List<Map> servers, String nomeVideo) {
    final isAtivo = server['url'] == _urlAtiva;
    final label = server['idioma'].toString();
    return GestureDetector(
      behavior: HitTestBehavior.opaque,
      onTap: () {
        if (isAtivo) return; // já está ativo
        // Guarda posição atual para continuar do mesmo ponto
  ...
  void _iniciarExoPlayer(String url, String tituloEpisodio, {VideoPlayerController? controllerPreinit}) async {
    ...
    _videoPlayerController?.dispose();
    setState(() { 
      _urlAtiva = url; // dentro do setState para forçar rebuild dos chips
      _showControls = false; 
      _isBuffering = false; 
      isPlaying = true; 
    ...
    // Guarda a posição para usar no seek (pode ter sido definida pelo chip de idioma)
