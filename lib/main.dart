...
void main() async {
  ...
  // Permite portrait E landscape (necessário para TV Box que é sempre landscape)
...
class _PlayerScreenState extends State<PlayerScreen> {
  ...
  Future<void> _abrirServidores(String idVideo, String nomeVideo, bool isParaDownload) async {
    ...
    if (!isParaDownload) {
      // NÃO descartar o controller aqui — o _iniciarExoPlayer faz isso depois do setState
      // para evitar o frame cinza entre destruir o player antigo e criar o novo
      setState(() { 
        isPlaying = true; 
        isServerLoading = true; 
        epAtivoNome = nomeVideo; 
        savedEpId = idVideo; 
      });
    }
    ...
    try {
      ...
      if (data['success'] == true && data['players'] != null) {
        ...
        List<Map> servers = players.map((p) {
          ...
          final urlLow = url.toLowerCase();
          final nameLow = name.toLowerCase();

          // Funil de 3 etapas: verifica SEMPRE URL e título (não só quando nome está vazio)
          // Etapa 1: É Dublado?
          String idioma;
          if (urlLow.contains("/dub/") || nameLow.contains("dub")) {
            idioma = "Dublado";
          // Etapa 2: É Legendado?
          } else if (urlLow.contains("/leg/") || nameLow.contains("leg")) {
            idioma = "Legendado";
          // Etapa 3: Plano B — usa o nome original
          } else {
            idioma = name.isNotEmpty ? name : "Opção";
          }
        ...
        if (isParaDownload) {
          ...
          final idiomaRaw = serverEscolhido['idioma'].toString().toLowerCase();
          if (idiomaRaw.contains('dublado')) {
            _idiomaAtivo = 'Dublado';
          } else if (idiomaRaw.contains('legendado')) {
            _idiomaAtivo = 'Legendado';
          } else {
            _idiomaAtivo = serverEscolhido['idioma'].toString();
          }
          // Usa fallback inteligente: tenta servidores automaticamente antes de mostrar erro
          await _iniciarExoPlayerComFallbackInteligente(servers, serverEscolhido, nomeVideo);
        }
      }
    } catch (e) { 
      if (!isParaDownload) setState(() { isServerLoading = false; isPlaying = false; });
  ...
  Widget _buildSeletorIdioma(List<Map> servers, String nomeVideo) {
    // Filtra apenas servidores reconhecidos como Dublado ou Legendado
    final dubServers = servers.where((s) => s['idioma'] == 'Dublado').toList();
    final legServers = servers.where((s) => s['idioma'] == 'Legendado').toList();

    // Só mostra se encontrou pelo menos um Dublado E um Legendado
    if (dubServers.isEmpty || legServers.isEmpty) return const SizedBox.shrink();

    // Um chip por idioma (pega o melhor servidor de cada — MP4 preferido)
    final serverDub = dubServers.firstWhere((s) => s['isMp4'] == true, orElse: () => dubServers.first);
    final serverLeg = legServers.firstWhere((s) => s['isMp4'] == true, orElse: () => legServers.first);
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 8),
        Row(children: [
          _chipServidor(serverDub, nomeVideo),
          const SizedBox(width: 8),
          _chipServidor(serverLeg, nomeVideo),
        ]),
        const SizedBox(height: 8),
      ],
    );
  ...
  Widget _chipServidor(Map server, String nomeVideo) {
    final isAtivo = server['url'] == _urlAtiva;
    final label = server['idioma'].toString(); // sempre "Dublado" ou "Legendado"
    return GestureDetector(
      behavior: HitTestBehavior.opaque,
      onTap: () async {
        if (isAtivo) return;
        final posAtual = _videoPlayerController?.value.position.inSeconds ?? 0;
        savedPositionSeconds = posAtual;
        setState(() => _idiomaAtivo = label);
        await _iniciarExoPlayer(server['url'], nomeVideo);
      },
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(6),
          border: Border.all(color: isAtivo ? Colors.transparent : Colors.white38),
        ),
        child: Text(label, style: TextStyle(color: isAtivo ? Colors.white : Colors.grey[300], fontSize: 13, fontWeight: FontWeight.bold)),
      ),
    );
  }

  Future<void> _iniciarExoPlayerComFallback(List<Map> servers, Map serverPrincipal, String nomeVideo) async {
    final idiomaRaw = serverPrincipal['idioma'].toString().toLowerCase();
    String idiomaLimpo;
    if (idiomaRaw.contains('dublado')) {
      idiomaLimpo = 'Dublado';
    } else if (idiomaRaw.contains('legendado')) {
      idiomaLimpo = 'Legendado';
    } else {
      idiomaLimpo = serverPrincipal['idioma'].toString();
    }
    setState(() { _idiomaAtivo = idiomaLimpo; });
    // Usa sistema de fallback inteligente: tenta servidores automaticamente antes de mostrar erro
    await _iniciarExoPlayerComFallbackInteligente(servers, serverPrincipal, nomeVideo);
  }
  ...
  void _mostrarUnityInterstitial({required VoidCallback onComplete}) {
    // Mostra anúncio dentro do app em vez de abrir janela separada do Unity
    // Isso evita que o utilizador burle fechando a janela externa
    showDialog(
      context: context,
      barrierDismissible: false,
      builder: (ctx) => _InterstitialInApp(onComplete: () {
        Navigator.pop(ctx);
        onComplete();
      }),
    );
  ...
  void _mostrarRewardedPopup() {
    ...
    // Bloqueia completamente — barrierDismissible: false já impede fechar
    // O vídeo só retoma após o utilizador VER o anúncio OU aguardar
    showDialog(
      context: context,
      barrierDismissible: false,
      useRootNavigator: true, // garante que fica por cima de tudo
      builder: (ctx) => PopScope(
        canPop: false, // bloqueia botão back físico
  ...
  // Contador de tentativas de fallback por sessão de reprodução
  int _fallbackAttempt = 0;
  List<Map> _fallbackQueue = [];
  String _fallbackNomeVideo = '';
  bool _fallbackEmAndamento = false;

  /// Inicia reprodução com fallback automático inteligente.
  /// Tenta cada servidor disponível antes de mostrar erro ao utilizador.
  Future<void> _iniciarExoPlayerComFallbackInteligente(
    List<Map> servers, Map serverPrincipal, String nomeVideo,
  ) async {
    _fallbackAttempt = 0;
    _fallbackNomeVideo = nomeVideo;
    // Monta fila: servidor principal primeiro, depois os restantes por ordem de qualidade
    _fallbackQueue = [
      serverPrincipal,
      ...servers.where((s) => s['url'] != serverPrincipal['url']).toList(),
    ];
    _fallbackEmAndamento = false;
    await _tentarProximoServidor();
  }

  Future<void> _tentarProximoServidor() async {
    if (!mounted) return;
    if (_fallbackAttempt >= _fallbackQueue.length) {
      // Esgotou todos os servidores — agora mostra erro
      if (mounted) {
        setState(() { isServerLoading = false; isPlaying = false; });
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Nenhum servidor disponível. Tenta mais tarde."),
    ...
    final server = _fallbackQueue[_fallbackAttempt];
    _fallbackAttempt++;
    await _iniciarExoPlayer(server['url'], _fallbackNomeVideo, isFallback: true);
  }

  Future<void> _iniciarExoPlayer(String url, String tituloEpisodio, {
    VideoPlayerController? controllerPreinit,
    ...
    if (!mounted) return;
    // Marca loading ANTES de descartar o controller antigo.
    // Assim o Flutter renderiza o spinner vermelho no lugar certo,
    // e NÃO há frame vazio/cinza entre destruir o player antigo e criar o novo.
    setState(() {
      _urlAtiva = url;
      _showControls = false;
      _isBuffering = false;
    ...
    // Guarda referências antigas para descartar DEPOIS do setState (evita flash cinza)
    final oldChewie = _chewieController;
    final oldVideo = _videoPlayerController;
    _chewieController = null;
    _videoPlayerController = null;
    // Descarta após um microtask — o frame do spinner já foi agendado
    Future.microtask(() { oldChewie?.dispose(); oldVideo?.dispose(); });

    final posParaSeek = savedPositionSeconds;

    // Detecta formato pelo URL — suporta MKV, MP4, HLS, AVI, MOV, WEBM, TS, etc.
    VideoFormat? formatHint;
    final urlLow = url.toLowerCase();
    if (urlLow.contains('.m3u8') || urlLow.contains('hls') || urlLow.contains('/m3u8')) {
    ...
    try {
      final controller = controllerPreinit ?? VideoPlayerController.networkUrl(
        Uri.parse(url),
        formatHint: formatHint,
        httpHeaders: {
          "Referer": smartPlayUrl,
      ...
      try {
        ...
        if (isFallback) {
          ...
          if (mounted) setState(() { isServerLoading = false; isPlaying = false; });
        }
        return;
      }

      if (!initOk || !mounted) { controller.dispose(); return; }
      ...
      _chewieController = ChewieController(
        ...
        // errorBuilder não dispara fallback — o listener abaixo é mais preciso
        errorBuilder: (ctx, msg) => Container(color: Colors.black),
      );

      // Listener de erros — com proteção contra falsos positivos durante seek/drag
      // Só aciona fallback se o vídeo NUNCA chegou a reproduzir (hasError logo no início)
      bool _jáReproduziu = false;
      bool _errorReportado = false;
      _videoPlayerController!.addListener(() {
        if (!mounted) return;
        final val = _videoPlayerController!.value;

        // Regista que o vídeo chegou a reproduzir pelo menos 1 segundo
        if (val.position.inSeconds >= 1) _jáReproduziu = true;

        // Buffering — só atualiza o indicador, nunca trata como erro
        final buf = val.isBuffering;
        if (buf != _isBuffering) setState(() => _isBuffering = buf);

        // Erro real — só acciona se:
        // 1. Nunca reproduziu (erro logo no início, não durante seek/drag)
        // 2. Não foi já reportado
        if (val.hasError && !_errorReportado && !_jáReproduziu) {
          _errorReportado = true;
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (!mounted) return;
            if (isFallback && _fallbackAttempt < _fallbackQueue.length) {
              _tentarProximoServidor();
            } else {
              // Vídeo nunca carregou — mostra mensagem discreta
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text("Erro ao reproduzir. Tenta outro idioma."),
      ...
      if (isFallback && mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text("Erro ao carregar o vídeo. Tenta outra opção."),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Widget _buildPlayerArea() {
    return Focus(
      autofocus: false, // não roubar foco automaticamente — evita tela branca
      onKeyEvent: (node, event) {
        ...
        if (key == LogicalKeyboardKey.goBack || key == LogicalKeyboardKey.escape) {
          if (_isFullscreen) { _exitFullscreen(); return KeyEventResult.handled; }
        }
      ...
      child: Stack(
        fit: StackFit.expand,
        children: [
          // Fundo preto SEMPRE presente — garante que nunca aparece cinza/branco do sistema
          Container(color: Colors.black),

          // Poster/backdrop quando não reproduz — só mostra imagem se URL não estiver vazia
          if (!isPlaying || isServerLoading) ...[
            if ((backdrop.isNotEmpty || widget.item['imagem']?.toString().isNotEmpty == true))
              CachedNetworkImage(
                imageUrl: backdrop.isNotEmpty ? backdrop : widget.item['imagem'],
                fit: BoxFit.cover,
          ...
          // Player Chewie — só renderiza quando completamente pronto
          if (isPlaying && !isServerLoading && _chewieController != null)
            Chewie(controller: _chewieController!),

          // Loading — enquanto inicializa o servidor
          if (isPlaying && isServerLoading)
            const Center(child: CircularProgressIndicator(color: Color(0xFFE50914))),
          ...
          // Botão play para filmes (antes de iniciar)
              ...
              children: [
                Container(
                  ...
                  child: AspectRatio(
                    aspectRatio: 16 / 9,
                    child: ColoredBox(
                      color: Colors.black, // garante fundo preto em qualquer estado
                      child: _buildPlayerArea(),
                    ),
                  ),
                ),
                // InAppWebView em Offstage — totalmente escondido mas mantido no tree
                // Offstage impede o SurfaceView de renderizar (evita flash cinza em Huawei/etc)
                Offstage(
                  offstage: true,
                  child: SizedBox(
                    height: 1, width: 1,
...
// ==========================================
// ==========================================
// INTERSTITIAL DENTRO DO APP (sem abrir janela separada)
// ==========================================
class _InterstitialInApp extends StatefulWidget {
  final VoidCallback onComplete;
  const _InterstitialInApp({required this.onComplete});
  @override State<_InterstitialInApp> createState() => _InterstitialInAppState();
}
...
class _InterstitialInAppState extends State<_InterstitialInApp> {
  ...
  @override void initState() {
    ...
    _timer = Timer.periodic(const Duration(seconds: 1), (t) {
      ...
      setState(() => _countdown--);
      if (_countdown <= 0) { t.cancel(); widget.onComplete(); }
    });
  }
      ...
      child: Dialog(
        ...
        child: SizedBox(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              // Banner de anúncio embutido
              SizedBox(
                height: 250,
                child: InAppWebView(
                  initialData: InAppWebViewInitialData(data: _c2),
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 8),
                    const Expanded(child: Text("Anúncio — obrigado por apoiar o CDCINE!", style: TextStyle(color: Colors.white54, fontSize: 12))),
                    if (_countdown > 0)
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                        decoration: BoxDecoration(color: Colors.white12, borderRadius: BorderRadius.circular(20)),
                        child: Text("$_countdown seg", style: const TextStyle(color: Colors.white60, fontSize: 13, fontWeight: FontWeight.bold)),
                      )
                    else
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: const Text("Fechar ✕", style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.bold)),
                        ),
                      ),
                  ],
                ),
          ...
          children: [
            ...
            Container(
              ...
              child: Column(children: [
                ...
                const SizedBox(height: 6),
                const Text("Segue estes passos simples para ver o teu conteúdo favorito no ecrã grande!", textAlign: TextAlign.center, style: TextStyle(color: Colors.white70, fontSize: 13, height: 1.5)),
              ]),
            ),
