...
class _PlayerScreenState extends State<PlayerScreen> {
  ...
  Future<void> _abrirServidores(String idVideo, String nomeVideo, bool isParaDownload) async {
    ...
    if (!isParaDownload) {
      // Dispõe imediatamente os controllers antigos para evitar tela branca
      _chewieController?.dispose();
  ...
  Future<void> _iniciarExoPlayer(String url, String tituloEpisodio, {
    ...
    // Dispose defensivo (caso chamado diretamente, ex: chip de idioma)
    _chewieController?.dispose();
    _chewieController = null;
    _videoPlayerController?.dispose();
    _videoPlayerController = null;
    ...
    if (!isFallback) {
      // Reset da fila de fallback quando iniciado manualmente (chip idioma, etc.)
      _fallbackAttempt = 0;
      _fallbackQueue = [];
    }
    ...
    if (urlLow.contains('.m3u8') || urlLow.contains('hls') || urlLow.contains('/m3u8')) {
      ...
      formatHint = VideoFormat.other; // suporta MKV, MP4, AVI, MOV, WEBM, TS e mais
    }
    ...
    try {
      ...
      // Timeout de inicialização: 20s. Se demorar mais, pode ser carregamento lento — não é erro.
      // Só cancela se realmente falhar.
      bool initOk = false;
      try {
        await controller.initialize().timeout(const Duration(seconds: 20));
        initOk = true;
      } on TimeoutException {
        // Timeout — tenta o próximo servidor silenciosamente (sem mostrar erro)
        controller.dispose();
        if (isFallback) {
          await _tentarProximoServidor();
        } else {
      ...
      // Seek correto: após initialize(), antes de criar o Chewie.
      // Isso evita tela preta ao avançar/retomar.
      if (posParaSeek > 0) {
        await _videoPlayerController!.seekTo(Duration(seconds: posParaSeek));
        // Aguarda o seek processar para evitar frame preto
      ...
      _chewieController = ChewieController(
        ...
        autoPlay: true,
        looping: false,
        // NÃO usar startAt — o seek já foi feito acima; startAt causa tela preta
        allowFullScreen: true,
        allowMuting: true,
        ...
        errorBuilder: (ctx, msg) {
          // Erro no Chewie — tenta próximo servidor automaticamente
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (mounted && isFallback) {
      ...
      // Listener: detecta buffering e erros reais do player
      bool _errorReportado = false;
      _videoPlayerController!.addListener(() {
        if (!mounted) return;
        final val = _videoPlayerController!.value;

        // Buffering — atualiza indicador mas NÃO trata como erro
        final buf = val.isBuffering;
        if (buf != _isBuffering) setState(() => _isBuffering = buf);

        // Erro real do codec/rede — faz fallback automático (uma só vez)
        if (val.hasError && !_errorReportado) {
          _errorReportado = true;
          if (isFallback && _fallbackAttempt < _fallbackQueue.length) {
            // Tenta próximo servidor silenciosamente
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (mounted) _tentarProximoServidor();
            });
          } else if (!isFallback) {
            // Iniciado manualmente sem fila — mostra snackbar discreto
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text("Erro ao reproduzir. Tenta outro idioma ou servidor."),
      ...
      // Erro na inicialização — tenta próximo da fila silenciosamente
