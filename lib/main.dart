...
class _PlayerScreenState extends State<PlayerScreen> {
  ...
  Widget _chipIdioma(String idioma, List<Map> servers, String nomeVideo) {
    // Normaliza para comparação: se o idioma ativo contém "legendado" e o chip é "Legendado", ativa
    final idiomaLower = idioma.toLowerCase();
    final isAtivo = _idiomaAtivo.toLowerCase().contains(idiomaLower);
    return GestureDetector(
      behavior: HitTestBehavior.opaque,
      onTap: () {
        final server = servers.firstWhere(
          (s) => s['idioma'].toString().toLowerCase().contains(idiomaLower) && s['isMp4'] == true,
          orElse: () => servers.firstWhere(
            (s) => s['idioma'].toString().toLowerCase().contains(idiomaLower),
            orElse: () => servers.first));
        if (server['url'] == _urlAtiva) return; // já está no idioma ativo

        // Captura a posição actual do vídeo a reproduzir para continuar do mesmo ponto
        final posAtual = _videoPlayerController?.value.position.inSeconds ?? 0;
        savedPositionSeconds = posAtual;

        setState(() => _idiomaAtivo = idioma); // usa o nome limpo do chip (Dublado/Legendado)
